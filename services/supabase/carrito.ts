...
export const carritoService = {
  ...
  async agregarAlCarrito(
    ...
    try {
      console.log(`[${new Date().toLocaleTimeString()}] Agregando al carrito:`, medicamento.id);
      ...
      console.log(`[${new Date().toLocaleTimeString()}] Agregado al carrito`);
  ...
  async limpiarCarrito(dni: string): Promise<boolean> {
    try {
      ...
      if (!error) console.log(`[${new Date().toLocaleTimeString()}] Carrito limpiado`);
      return !error;
