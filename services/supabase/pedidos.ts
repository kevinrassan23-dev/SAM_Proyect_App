...
export const pedidosService = {
  ...
  async crearPedidoCompleto(
    ...
    try {
      console.log(`[${new Date().toLocaleTimeString()}] Creando pedido completo...`);
      ...
      if (errorPedido) {
        console.error(`[${new Date().toLocaleTimeString()}] Error creando pedido:`, errorPedido);
        return { exito: false, mensaje: 'Error al crear el pedido' };
      }

      const pedidoId = pedido[0].ID_Pedido;
      console.log(`[${new Date().toLocaleTimeString()}] Pedido creado:`, pedidoId);
      ...
      if (errorItems) {
        console.error(`[${new Date().toLocaleTimeString()}] Error creando items:`, errorItems);
        return { exito: false, mensaje: 'Error al guardar items del pedido' };
      }

      console.log(`[${new Date().toLocaleTimeString()}] Items del pedido guardados`);
      ...
      if (errorTransaccion) {
        console.error(`[${new Date().toLocaleTimeString()}] Error creando transacción:`, errorTransaccion);
        return { exito: false, mensaje: 'Error al crear la transacción' };
      }

      console.log(`[${new Date().toLocaleTimeString()}] Transacción creada`);
      ...
      if (errorHistorial) {
        console.error(`[${new Date().toLocaleTimeString()}] Error creando historial:`, errorHistorial);
      }

      console.log(`[${new Date().toLocaleTimeString()}] Historial registrado`);
  ...
  async completarPedido(pedidoId: string): Promise<boolean> {
    try {
      ...
      console.log(`[${new Date().toLocaleTimeString()}] Pedido completado`);
      return true;
  ...
  async cancelarPedido(pedidoId: string, motivo: string): Promise<boolean> {
    try {
      ...
      console.log(`[${new Date().toLocaleTimeString()}] Pedido cancelado`);
      return true;
